...
if (isset($_GET['pID']))
{
    $stmt = $conn->prepare("select Procedures.procID, Procedures.procedure_name, Person.first_name, Person.last_name from Person, Patient, Procedures, Visit where Patient,pID=$_GET[pID] Procedures.visitID = Visit.visitID and Visit.pID = Patient.pID and Person.SSN = Patient.SSN order by Procedures.procID");
    $stmt->execute();
